...
feature set over HTTP so a custom UI can use it as a backend. ~125 endpoints
...
`collection_handle.py`, `config.py`, `errors.py`, `ids.py`, `schemas/common.py`
(OpChanges + Mutation), and one file per domain in `routers/`. Tests mirror under
`tests/unit` and `tests/e2e` (one `test_<domain>.py` each); `tests/conftest.py`
has the `api`/`handle`/`settings` fixtures and a `V1Client` wrapper.
